...
{markup_table}
  ...
  <div class="section">
    <h2>3. The open-source escape hatch</h2>
    <p>If overage charges and billing lock-in are a concern, the cleanest exit is switching to an open-source, self-hosted tool. {_html_escape(sh_names)} are free, MIT/Apache/AGPL-licensed software — the only recurring cost is the server it runs on.</p>
  ...
  <div class="section">
    ...
    faq = [
        {"q": "What happens if I exceed my automation plan limit?",
         "a": ("It depends on the tool. Most cloud platforms use tier-based pricing: exceeding your monthly "
               ...
               "Self-hosted open-source tools (n8n, Activepieces, Automatisch, Node-RED) have no hard ops "
               "ceiling — you're limited only by your server capacity.")},
        {"q": "Is n8n really free?",
         "a": ("n8n the software is open-source (Sustainable Use license) and free to self-host. You pay "
        ...
        {"q": "How do I avoid automation overage charges?",
         "a": ("Three strategies work reliably: (1) Monitor your run volume before it hits the limit — "
               "most platforms send alerts. (2) Self-host on open-source software (n8n, Activepieces) — "
    ...
    desc = ("Zapier, Make and Pipedream overage behavior, annual billing premiums and volume tier traps — "
            "everything that's not on the pricing page. Plus the open-source tools that cost only your server.")
