...
# index.html is the promoted live homepage (commit b5f2e75 promoted home-v6.html -> index.html).
# The DATA:* marker blocks are regenerated IN-PLACE here; everything outside the markers
# (the dashboard design) is preserved verbatim. Override target with `python build_home_v6.py <file>`.
HOME = ROOT / (sys.argv[1] if len(sys.argv) > 1 else "index.html")
